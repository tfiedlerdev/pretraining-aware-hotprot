...
class CNNPregeneratedFC(nn.Module):
    def __init__(self, input_seq_len=700, num_hidden_layers=1, first_hidden_size=512):
        ...
        self.thermo_cnn = nn.Sequential(
            ...
            nn.Conv2d(in_channels=8, out_channels=output_channels, kernel_size=3, stride=(1,1)),
            nn.MaxPool2d(kernel_size=(2,2), stride=(2,2)),
            nn.Conv2d(in_channels=output_channels, out_channels=output_channels, kernel_size=3, stride=(1,1)),
            nn.MaxPool2d(kernel_size=(2,2), stride=(2,2)),
            nn.Conv2d(in_channels=output_channels, out_channels=output_channels, kernel_size=3, stride=(1,1)),
            nn.MaxPool2d(kernel_size=(2,2), stride=(2,2)),
        )
